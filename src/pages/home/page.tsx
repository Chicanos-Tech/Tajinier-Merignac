...
export default function Home() {
  // New flow: email → wheel → review → result
  const [step, setStep] = useState<'email' | 'wheel' | 'review' | 'result'>('email');
  ...
  // -------------------------------------------------------------------------
  // RESET: Always start fresh, no localStorage check
  // -------------------------------------------------------------------------
  useEffect(() => {
    // Clear any previous session data on mount
    try {
      localStorage.removeItem('currentStep');
      localStorage.removeItem('completedEmail');
      localStorage.removeItem('wonPrize');
      localStorage.removeItem('reviewConfirmed');
    } catch (e) {
  ...
  const handleSpinComplete = (selectedPrize: string) => {
    ...
    try {
      ...
      console.error('Error logging prize', e);
    }
  ...
  // -------------------------------------------------------------------------
  return (
    <div className="min-h-[100dvh] relative overflow-hidden flex flex-col items-center justify-center px-3 py-2">
      {/* Fond Marrakech */}
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        ...
        <div className="relative w-full mt-2">
          ...
          <div className="relative bg-gradient-to-b from-[#1A0A0E] via-[#200E14] to-[#1A0A0E] rounded-2xl p-4 sm:p-5 border border-[#C9A227]/40 shadow-2xl">
            ...
            {/* STEP 2: WHEEL (maintenant AVANT l'avis) */}
            {step === 'wheel' && (
              <div className="text-center space-y-2">
                ...
                <p className="text-[#E8DFD3]/70 text-xs">
                  Tentez votre chance et découvrez votre cadeau
                </p>
                <WheelCanvas
                ...
                <button
                  ...
                  className={`w-full font-semibold py-3 px-6 rounded-xl transition-all duration-300 transform shadow-lg text-xs whitespace-nowrap ${
                    isSpinning || hasSpun
                      ? 'bg-gray-600 cursor-not-allowed text-gray-400 opacity-60'
                      : 'bg-gradient-to-r from-[#1B4D3E] to-[#2D5F4F] hover:from-[#163D32] hover:to-[#265248] text-white hover:scale-[1.02] border border-[#C9A227]/30 cursor-pointer'
                  }`}
                >
                  {isSpinning ? (
                    <>
            ...
            {/* STEP 3: REVIEW (APRÈS la roue) */}
            {step === 'review' && (
              <div className="text-center space-y-3">
                {/* Affichage du gain */}
                <div className="bg-gradient-to-r from-[#1B4D3E]/20 via-[#2D5F4F]/20 to-[#1B4D3E]/20 rounded-xl p-3 border border-[#C9A227]/30 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#8B1538] via-[#C9A227] to-[#1B4D3E]" />
                  <p className="text-[10px] text-[#E8DFD3]/60 mb-1">Vous avez gagné :</p>
                  <p className="text-lg font-bold text-[#C9A227]" style={{ fontFamily: 'Playfair Display, serif' }}>
                    🎁 {prize}
                  </p>
                </div>

                <div className="w-12 h-12 mx-auto bg-gradient-to-br from-[#C9A227]/20 to-[#D4AF37]/10 rounded-full flex items-center justify-center border-2 border-[#C9A227]/40">
                  <i className="ri-star-fill text-xl text-[#C9A227]" />
                </div>

                <h2 className="text-base font-bold text-[#C9A227]" style={{ fontFamily: 'Playfair Display, serif' }}>
                  Dernière étape !
                </h2>
                <p className="text-[#E8DFD3]/80 text-xs leading-relaxed px-1">
                  Pour récupérer votre cadeau, laissez un avis Google. Une preuve du gain sera demandée au restaurant.
                </p>
                ...
                {!reviewConfirmed ? (
                  ...
                  >
                    ...
                    <i className="ri-external-link-line ml-2 opacity-60" />
                  </button>
                ) : (
                  <div className="space-y-2">
                    <div className="bg-[#1B4D3E]/30 border border-[#C9A227]/40 rounded-xl p-3 flex items-center justify-center gap-2">
                      <i className="ri-check-double-line text-[#C9A227] text-lg" />
                      <span className="text-[#E8DFD3]/80 text-xs">Avis ouvert dans un nouvel onglet</span>
                    </div>
                    <button
                      onClick={handleConfirmReview}
                      className="w-full bg-gradient-to-r from-[#1B4D3E] to-[#2D5F4F] hover:from-[#163D32] hover:to-[#265248] text-white font-semibold py-3.5 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg text-sm whitespace-nowrap cursor-pointer border border-[#C9A227]/30 animate-pulse"
                    >
                      <i className="ri-gift-2-line mr-2" />
                      J'ai laissé mon avis - Récupérer mon cadeau !
                    </button>
                    <button
                      onClick={handleOpenReview}
                      className="text-[#E8DFD3]/50 text-[10px] hover:text-[#C9A227] transition-colors cursor-pointer underline"
                    >
                      Rouvrir la page d'avis Google
                    </button>
                  </div>
                )}
              </div>
